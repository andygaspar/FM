...
void SPR(tree* T, double** D, double **A, int *count)
{
	...
	std::cout<<"Assign BME weights and re-weight tree.\n"<<std::endl;
	...
	std::cout<< "\tBefore SPR:     tree length is "<< T->weight<<std::endl;
	...
	} while (swapValue < -FLT_EPSILON && countMinWeight < 1);
